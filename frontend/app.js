...
// ── Audio alert (unlocked on first user click, per browser autoplay rules)
...
function addCanRows(frames) {
  const list = $("can-list");
  if (!list || !Array.isArray(frames)) return;
...
window.setScenario = async (s) => {
  ...
  const btnNormal = $("btn-normal"), btnAttack = $("btn-attack");
  if (btnNormal) btnNormal.classList.toggle("active", s === "normal");
  if (btnAttack) btnAttack.classList.toggle("active", s === "attack");

  if (s === "normal") {
    const banner = $("intrusion-banner");
  ...
  try {
    ...
    console.error("setScenario failed:", err);
    addLog("crit", "Failed to switch scenario — backend unreachable");
  }
};
...
function initCharts() {
  const obdCanvas = $("obd-chart");
  const canCanvas = $("can-chart");
  if (!obdCanvas || !canCanvas || typeof Chart === "undefined") {
    console.warn("Charts not initialised — canvas or Chart.js missing.");
    return;
  }

  obdChart = new Chart(obdCanvas.getContext("2d"), {
    type: "line",
    data: {
      labels,
      datasets: [
  ...
  canChart = new Chart(canCanvas.getContext("2d"), {
    type: "line",
    data: {
      labels,
      datasets: [
...
async function fetchAndUpdate() {
  try {
    ...
    if (apiStatus) {
      ...
      apiStatus.style.color = "#00e87a";
    }
    const tickTime = $("tick-time");
    if (tickTime) tickTime.textContent = nowStr();
    ...
    const msObd  = $("ms-obd");
    const msCan  = $("ms-can");
    const msComp = $("ms-comp");
    if (msObd)  msObd.textContent  = fault_pct.toFixed(1) + "%";
    if (msCan)  msCan.textContent  = anom_pct.toFixed(1)  + "%";
    if (msComp) msComp.textContent = composite.toFixed(1) + "%";

    // Status dots
    const obdDot = $("obd-dot");
    const canDot = $("can-dot");
    if (obdDot) obdDot.className = "status-dot" + (fault_pct > 65 ? " crit" : fault_pct > 35 ? " warn" : "");
    if (canDot) canDot.className = "status-dot" + (anom_pct  > 60 ? " crit" : anom_pct  > 25 ? " warn" : "");
    ...
    const ssTime   = $("ss-time");
    const ssFrames = $("ss-frames");
    const ssAnom   = $("ss-anom");
    if (ssTime)   ssTime.textContent   = `${Math.floor(e / 60)}:${fmt2(e % 60)}`;
    if (ssFrames) ssFrames.textContent = session.frames_per_s;
    if (ssAnom)   ssAnom.textContent   = session.anom_frames;

    // Logs (throttled — only log meaningful state, not every tick)
    ...
    // Intrusion banner (fires once per episode)
    ...
    } else if (anom_pct <= 60 && intrusionShown) {
      // reset so banner can re-trigger on the next spike
      intrusionShown = false;
    }
